...
type cacheVerifier struct {
	cachedClient    client.PackageV1Alpha1Client
	nonCachedClient client.PackageV1Alpha1Client
	lister          verifyLister
	mutex           sync.Mutex
	diffMap         map[string]diffMapEntry
...
func newVerifier(restConfig *rest.Config, lister verifyLister) *cacheVerifier {
	return &cacheVerifier{
		mutex:      sync.Mutex{},
		diffMap:    make(map[string]diffMapEntry),
...
func (verifier *cacheVerifier) listCachedItems(ctx context.Context) ([]metav1.Object, error) {
	return verifier.lister(ctx, verifier.cachedClient)
}

func (verifier *cacheVerifier) listNonCachedItems(ctx context.Context) ([]metav1.Object, error) {
	return verifier.lister(ctx, verifier.nonCachedClient)
}
...
func (verifier *cacheVerifier) setClients(cachedClient client.PackageV1Alpha1Client, nonCachedClient client.PackageV1Alpha1Client) {
	verifier.cachedClient = cachedClient
	verifier.nonCachedClient = nonCachedClient
}

func (verifier *cacheVerifier) start(ctx context.Context, cachedClient client.PackageV1Alpha1Client) {
	// setup client without cache
	nonCachedClient := client.NewOrDie(verifier.restConfig)
	verifier.setClients(cachedClient, nonCachedClient)

	tick := time.NewTicker(time.Second * time.Duration(8*(rand.Float32()+1))) // between 8 - 16 seconds
	tickC := tick.C
	defer tick.Stop()
...
endlessLoop:
	for range tickC {
		if nonCachedItems, err := verifier.listNonCachedItems(ctx); err != nil {
			fmt.Fprintf(os.Stderr, "CACHEVERIFIER: failed to get actual items: %v\n", err)
		} else if cachedItems, err := verifier.listCachedItems(ctx); err != nil {
				...
				for _, cachedItem := range cachedItems {
					if nonCachedItem.GetNamespace() == cachedItem.GetNamespace() &&
						...
						if nonCachedItem.GetResourceVersion() != cachedItem.GetResourceVersion() {
							...
							verifier.mutex.Lock()
							delete(verifier.diffMap, cache.MetaObjectToName(cachedItem).String())
...
func (verifier *cacheVerifier) closeAndReset(err error) {
	...
	clear(verifier.diffMap)
	verifier.errCh <- err
}

func (verifier *cacheVerifier) newStopCh() chan struct{} {
	verifier.stopCh = make(chan struct{})
	return verifier.stopCh
}
...
func (verifier *cacheVerifier) handleDiff(cachedObj metav1.Object, nonCachedObj metav1.Object) error {
	...
	if lastDiff, exists := verifier.diffMap[diffMapKey]; exists {
		...
		// else: cached item has been updated since last time, but is not yet update, which is okay
		// we consider this as not being stuck (yet), but mark for check in next round
	}
	// mark for check in the next verification round
	verifier.diffMap[diffMapKey] = diffMapEntry{
		cachedResourceVersion:    cachedResourceVersion,
		nonCachedResourceVersion: nonCachedResourceVersion,
	}
	return nil
